...
/**
 * Función para generar nombres únicos para todos los empleados
 */
async function generateUniqueNamesForAll() {
  try {
    console.log('🔍 Iniciando generación de nombres únicos para todos los empleados...');
    ...
    // 2. Generar nombres únicos para todos los registros
    let updatedCount = 0;
    let errorCount = 0;
    const errors = [];
    
    // Usar un Set para rastrear nombres ya utilizados
    const usedNames = new Set();
    
    for (const record of allRecords) {
      try {
        // Generar un nombre único
        let uniqueName = generateUniqueName(record.employee_name, record.employee_email);
        
        // Si el nombre ya existe, añadir un sufijo numérico
        let counter = 1;
        while (usedNames.has(uniqueName)) {
          uniqueName = `${generateUniqueName(record.employee_name, record.employee_email)} ${counter}`;
          counter++;
        }
        
        // Añadir el nombre al conjunto de nombres utilizados
    ...
    console.error('❌ Error generando nombres únicos:', error);
    return {
      error: error.message
    };
  }
...
// Ejecutar la función
generateUniqueNamesForAll().then(result => {
