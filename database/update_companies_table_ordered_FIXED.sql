-- =====================================================
-- ACTUALIZACIÓN DE TABLA COMPANIES PARA FLUJO ORDENADO (CORREGIDO)
-- =====================================================
-- Fecha: 2025-11-25
-- Propósito: Agregar campos token_id y carpeta_id para el nuevo flujo ordenado
-- Estructura: Gmail/No-Gmail automática
-- CORRECCIÓN: Compatible con esquema existente

-- 1. AGREGAR NUEVOS CAMPOS A LA TABLA COMPANIES
ALTER TABLE companies 
ADD COLUMN IF NOT EXISTS token_id VARCHAR(64) UNIQUE,
ADD COLUMN IF NOT EXISTS carpeta_id VARCHAR(32) UNIQUE,
...
-- 9. CREAR VISTA PARA CONSULTAS OPTIMIZADAS (SIN drive_folder_id)
...
BEGIN
  ...
  SELECT 
    ...
    COUNT(*) FILTER (
      WHERE gmail_folder_id IS NOT NULL 
...
DO $$
BEGIN
  RAISE NOTICE '✅ Actualización de tabla companies completada (CORREGIDA)';
  ...
  RAISE NOTICE '🔧 CORRECCIÓN: Script compatible con esquema existente';
END $$;