...
const WhatsAppOfficialConfig = ({ onSave, onCancel }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      await whatsappOfficialService.saveConfiguration({
      ...
      toast.success('Configuración de WhatsApp Official API guardada')
      onSave(config)
    } catch (error) {
      console.error('Error saving WhatsApp Official config:', error)
  ...
  const handleTestConnection = async () => {
    try {
      ...
      // Probar la conexión con la API oficial de WhatsApp
      const result = await whatsappOfficialService.testConnection({
        accessToken: config.accessToken,
        phoneNumberId: config.phoneNumberId,
        testMode: config.testMode
      })
      ...
      if (result.success) {
        toast.success('Conexión con WhatsApp Official API exitosa')
      } else {
        toast.error(`Error de conexión: ${result.message}`)
      }
    } catch (error) {
      console.error('Error testing WhatsApp Official connection:', error)
      toast.error('Error al probar la conexión con WhatsApp Official API')
    } finally {
      setTesting(false)
    }
  }
  ...
  return (
    <div className="bg-white rounded-3xl p-6 shadow-xl border border-gray-100">
      <h3 className="text-xl font-bold text-gray-900 mb-4">Configuración de WhatsApp Official API</h3>
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Token de acceso a la API oficial de WhatsApp
          </p>
        </div>
        ...
        <div>
          ...
          />
          <p className="text-xs text-gray-500 mt-1">
            Token para verificar webhooks entrantes (opcional)
          </p>
        </div>
        ...
        <div className="flex items-center">
          ...
          <label htmlFor="enabled" className="ml-2 block text-sm text-gray-700">
            Habilitar integración con WhatsApp Official API
          </label>
        </div>
...
export default WhatsAppOfficialConfig