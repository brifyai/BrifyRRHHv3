 ...
 * Usa supabaseServer.js para compatibilidad con Node.js
 * Ejecutar: node seed_companies_server.mjs
 */
...
async function seedCompanies() {
  ...
  try {
    ...
    const { data: existingCompanies, error: checkError } = await supabaseServer
      .from('companies')
      .select('id')
      .limit(1)
    ...
    if (existingCompanies && existingCompanies.length > 0) {
      ...
      console.log('   Ejecuta: node seed_companies_server.mjs --force')
      return
    }
    ...
    // Insertar empresas
    const { data: insertedCompanies, error: insertError } = await supabaseServer
      .from('companies')
      .insert(companies)
      .select()
    ...
    console.log('   node -e "import(\"./src/lib/supabaseServer.js\").then(m => m.supabaseServer.from(\"companies\").select().then(r => console.log(r.data)))"')
